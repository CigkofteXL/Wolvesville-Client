...
;(function () {
 ...
 const messagesToCatch = {
      'game-joined': (data) => {
          ...
          YSS_SOCKET.send(GAME_ID ? JSON.stringify({ type: "ROOM_CREATED", gameId: GAME_ID }) : null);
      },
      'lobby:chat-msg': (data) => {
  ...
  const Join = (gameid) => {
    ...
    if (!PLAYER || !PLAYER.username) {
      ...
      if (GAME_STATUS === 'started' || GAME_ID !== undefined) {
          return;
      }
  ...
  const UseSkill = (payloadStr) => {
      // payloadStr: C#'tan gelen detaylı skill verisi
      // Örn: { role: "seer", target: "id123" } veya { role: "priest", target: "id123" }
      try {
          ...
          switch(roleId) {
              case "seer":
              case "aura-seer":
                  emitNative(`game-${roleId}-view-role`, JSON.stringify({ targetPlayerId: targetId }));
                  break;
              case "priest":
              ...
              case "gunner":
                  ...
                  break;
              case "detective":
                  // Detective 2 hedef bekler (targetId array olarak gelir)
                  if(Array.isArray(targetId)) {
                      emitNative('game-detective-selected-targets', JSON.stringify(targetId));
                  }
                  break;
              case "gambler":
                  emitNative('game-gambler-selected-player', JSON.stringify({ targetPlayerId: targetId, teamGuess: 'villager', teamName: 'Village', teamNameEn: 'Village' }));
                  break;
              case "mayor":
...
// HBV Core
function startHBVCore() {
  console.log('Halid bin velid (r.a) feth etti');
  if(window.VarietyshopAPI) {
      window.VarietyshopAPI.log('HBV Interceptor devrede. Soket trafiği dinleniyor.', 'INFO');
  }
  // Geri kalan soket hook ve auth mantıkları buraya eklenecek.
}
//HBV kodları
;(function () {
